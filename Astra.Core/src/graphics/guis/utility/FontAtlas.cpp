...
namespace Astra::Graphics
{
        ...
        : m_fontSize(fontSize)
    {
		...
		{
			Logger::LogError("FreeType: Could not initialize library");
			return;
		}
		...
		{
			Logger::LogError(std::string("Failed to load font: ") + std::string(fontFile));
			return;
		}
    ...
    const Post_Char& FontAtlas::GetCharacter(char c) const
    {
        ...
        {
            Logger::LogWarning("Font Atlas does not contain " + c);
        }
        return found->second;
    }
    ...
    void FontAtlas::LoadCharacter(const FT_Face& face, unsigned char c)
    {
        ...
        {
            Logger::LogError("FreeType: Could not load glyph " + c);
        }
    }
