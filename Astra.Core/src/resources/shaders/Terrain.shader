...
void main()
{
	...
	vec4 blendMapColor = texture(material.blendMap, v_TexCoordinates);
	float backTextureAmount = 1 - (blendMapColor.r + blendMapColor.g + blendMapColor.b);
	vec2 tiledCoords = v_TexCoordinates * 40;
	vec4 backgroundTextureColor = texture(material.backgroundTexture, tiledCoords) * backTextureAmount;
	vec4 rTextureColor = texture(material.rTexture, tiledCoords) * blendMapColor.r;
	vec4 gTextureColor = texture(material.gTexture, tiledCoords) * blendMapColor.g;
	vec4 bTextureColor = texture(material.bTexture, tiledCoords) * blendMapColor.b;

	vec4 totalColor = backgroundTextureColor + rTextureColor + gTextureColor + bTextureColor;
	vec3 color = totalColor.rgb;
