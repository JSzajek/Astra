...
namespace Astra::Graphics
{
	...
	Gui::Gui(const char* const name, const Math::Vec2& position, float rotation, const Math::Vec2& scale)
		: Spatial2D(name, position, rotation, scale), m_rect(m_rows[0].x, m_rows[0].y, m_rows[1].x, m_rows[1].y)
	{
	}
	...
	Gui::Gui(const Math::Vec2& position, float rotation, const Math::Vec2& scale)
		: Spatial2D(position, rotation, scale), Material(), m_rect(m_rows[0].x, m_rows[0].y, m_rows[1].x, m_rows[1].y)
	{
	}
